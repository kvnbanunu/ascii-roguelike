...
def heal(amount):
    ...
    if HP + amount < HPMAX:
        ...
        HP = HPMAX
    print(f"{name}'s HP refilled to {str(HP)}!")
...
def battle():
    ...
    while fight:
        ...
        print(f"{name}'s HP: {str(HP)}/{str(HPMAX)}")
        print(f"POTIONS: {str(pot)}")
        print(f"ELIXIR: {str(elix)}")
        draw()
        print("1 - ATTACK")
        if pot > 0:
        ...
        if choice == "1":
            hp -= ATK
            print(f"{name} dealt {str(ATK)} damage to the {enemy}.")
            if hp > 0:
                HP -= atk
                print(f"{enemy} dealt {str(atk)} damage to {name}.")
            input("> ")
...
while run:
    ...
    while play:
        ...
        if play:
            draw()
            print(f"LOCATION: {biom[map[y][x]]['t']}")
            draw()
            print(f"NAME: {name}")
            print(f"HP: {str(HP)}/{str(HPMAX)}")
            print(f"ATK: {str(ATK)}")
            print(f"POTIONS: {str(pot)}")
            print(f"ELIXIRS: {str(elix)}")
            print(f"GOLD: {str(gold)}")
